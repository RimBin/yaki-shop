...
export default function OrdersPage() {
  ...
  useEffect(() => {
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        if (res.status === 401) {
          if (!isCancelled) router.push(`${toLocalePath('/login', currentLocale)}?redirect=${encodeURIComponent(toLocalePath('/account/orders', currentLocale))}`);
          return;
        }
  ...
  }, [currentLocale, router, supabase, t, tCommon]);
  ...
  return (
    <AccountLayout active="orders" onLogout={handleLogout}>
      ...
      <div className="mb-6 flex items-center gap-3 md:hidden">
        <Link
          href={toLocalePath('/account/details', currentLocale)}
                            ...
                            {item.imageKey ? (
                              <div className="relative h-[64px] w-[64px] overflow-hidden rounded-[12px] bg-[#EAEAEA]">
                                <SeoImage
                                  ...
                                  alt={buildProductImageSeo(currentLocale, { name: item.name }).alt}
                                  title={buildProductImageSeo(currentLocale, { name: item.name }).title}
                                  description={buildProductImageSeo(currentLocale, { name: item.name }).description}
                                  fill
