...
export default function EmailTemplatesAdmin() {
  ...
  useEffect(() => {
    ...
    const verifyAccess = async () => {
      ...
      if (!token) {
        router.replace(toLocalePath('/login', currentLocale));
        return;
      }

      const response = await fetch('/api/admin/access?section=email-templates', {
        headers: {
  ...
  if (!isAccessResolved) {
    return (
      <AdminBody className="pt-[clamp(16px,2vw,24px)]">
        <AdminCard>
          <p className="font-['Outfit'] text-[14px] text-[#535353]">Tikrinamos prieigos teisės...</p>
        </AdminCard>
      </AdminBody>
    );
  }
    ...
    () =>
      isLt
        ? {
            ...
            providerNote:
              'Pastaba: siuntimui reikia el. pašto tiekėjo konfigūracijos. Galite naudoti RESEND_API_KEY arba SMTP_HOST/SMTP_PORT/SMTP_USER/SMTP_PASS. Jei norite priverstinai naudoti SMTP, nustatykite EMAIL_PROVIDER=smtp.',
            ...
            testSendOk: 'Testinis laiškas sėkmingai išsiųstas.',
        ...
        : {
            ...
            providerNote:
              'Note: sending requires email provider config. You can use RESEND_API_KEY or SMTP_HOST/SMTP_PORT/SMTP_USER/SMTP_PASS. To force SMTP, set EMAIL_PROVIDER=smtp.',
            ...
            testSendOk: 'Test email sent successfully.',
