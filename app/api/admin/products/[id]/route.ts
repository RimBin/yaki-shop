...
const LEGACY_THERMO_WOOD_TYPE = 'thermo'

function isStandaloneThermoWoodType(value: unknown): boolean {
  return String(value || '').trim().toLowerCase() === LEGACY_THERMO_WOOD_TYPE
}
...
async function getExistingWoodType(supabase: Awaited<ReturnType<typeof requireAdmin>>['supabase'], id: string) {
  ...
  if (error || !data) {
    return { woodType: null, error: error?.message || 'Product not found' }
  }

  return { woodType: data.wood_type as string | null, error: null }
}
...
export async function GET(
  ...
  try {
    const { supabase } = await requireAdmin(request, 'products')
...
export async function PATCH(
  ...
  try {
    ...
    const existing = await getExistingWoodType(supabase, id)
    if (existing.error) {
      return NextResponse.json({ error: existing.error }, { status: 404 })
    }

    if (
      body.woodType !== undefined &&
      isStandaloneThermoWoodType(body.woodType) &&
      !isStandaloneThermoWoodType(existing.woodType)
    ) {
      return NextResponse.json(
        { error: 'Thermo can no longer be saved as a standalone admin product' },
        { status: 400 }
      )
    }
...
export async function PUT(
  ...
  try {
    ...
    if (
      body.wood_type !== undefined &&
      isStandaloneThermoWoodType(body.wood_type) &&
      !isStandaloneThermoWoodType(existing.woodType)
    ) {
      return NextResponse.json(
        { error: 'Thermo can no longer be saved as a standalone admin product' },
