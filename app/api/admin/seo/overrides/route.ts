...
function normalizeCanonicalPath(input: string): string {
  ...
  // Accept either a pathname (/foo) or a full URL (https://shop.yakiwood.co.uk/foo)
  try {
...
export async function GET(request: NextRequest) {
  try {
    const { supabase } = await requireAdmin(request, 'seo')
...
export async function PUT(request: NextRequest) {
  try {
    const { supabase, user } = await requireAdmin(request, 'seo')
