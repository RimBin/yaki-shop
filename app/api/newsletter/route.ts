...
import { isNewsletterDemoModeEnabled } from '@/lib/server/runtime-flags';
import { z } from 'zod';

const FORCE_NEWSLETTER_UI_SUCCESS = true;
...
function buildDemoSuccessResponse() {
...
function shouldReturnSuccessFallback(request: NextRequest) {
  return FORCE_NEWSLETTER_UI_SUCCESS || isNewsletterDemoModeEnabled(request);
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (shouldReturnSuccessFallback(request)) {
        return buildDemoSuccessResponse();
      }
      return NextResponse.json(
        {
          success: false,
    ...
    if (result.success) {
      ...
      if (shouldReturnSuccessFallback(request)) {
        console.warn('Newsletter demo fallback used after provider failure:', result.error);
        return buildDemoSuccessResponse();
      }
      return NextResponse.json(
        {
