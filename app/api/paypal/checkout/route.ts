...
export async function POST(req: NextRequest) {
  try {
    ...
    const resolvedSiteUrl = siteUrl && siteUrl.trim() ? siteUrl.trim().replace(/\/$/, '') : 'https://shop.yakiwood.co.uk'
    ...
    const paypalData = (await paypalOrderRes.json().catch(() => null)) as PayPalCreateOrderResponse | null
    ...
    const approvalLink = Array.isArray(paypalData?.links)
      ? paypalData.links.find((link) => link?.rel === 'approve')
      : null
