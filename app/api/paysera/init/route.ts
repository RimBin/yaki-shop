...
function getPayseraConfig() {
  ...
  const resolvedSiteUrl =
    siteUrl && siteUrl.trim().length > 0 ? siteUrl.trim().replace(/\/$/, '') : 'https://shop.yakiwood.co.uk'

  const version = (process.env.PAYSERA_VERSION || '1.6').trim()
  const rawTest = typeof process.env.PAYSERA_TEST === 'string' ? process.env.PAYSERA_TEST.trim().toLowerCase() : ''
  const test = rawTest === '1' || rawTest === 'true' || rawTest === 'yes' || rawTest === 'on'
