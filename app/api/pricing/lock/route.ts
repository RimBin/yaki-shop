...
import { generateQuoteToken, getQuoteTtlMinutes, hashQuoteToken, hasQuoteTokenSecret } from '@/lib/pricing/quote-token'
  ...
  try {
    ...
    const items = itemsRaw
      .map((item) => ({
        ...
        inputMode: item.inputMode === 'area' ? 'area' : item.inputMode === 'boards' ? 'boards' : undefined,
        targetAreaM2: typeof item.targetAreaM2 === 'number' ? item.targetAreaM2 : undefined,
        pricingSnapshot:
          item && typeof (item as any).pricingSnapshot === 'object' && (item as any).pricingSnapshot !== null
            ? {
                unitAreaM2:
                  typeof (item as any).pricingSnapshot.unitAreaM2 === 'number'
                    ? (item as any).pricingSnapshot.unitAreaM2
                    : undefined,
                totalAreaM2:
                  typeof (item as any).pricingSnapshot.totalAreaM2 === 'number'
                    ? (item as any).pricingSnapshot.totalAreaM2
                    : undefined,
                pricePerM2Used:
                  typeof (item as any).pricingSnapshot.pricePerM2Used === 'number'
                    ? (item as any).pricingSnapshot.pricePerM2Used
                    : undefined,
                unitPrice:
                  typeof (item as any).pricingSnapshot.unitPrice === 'number'
                    ? (item as any).pricingSnapshot.unitPrice
                    : undefined,
                lineTotal:
                  typeof (item as any).pricingSnapshot.lineTotal === 'number'
                    ? (item as any).pricingSnapshot.lineTotal
                    : undefined,
              }
            : undefined,
        configuration:
    ...
    const pricedLines = await Promise.all(
      items.map(async (item) => {
        ...
        const inputMode = item.inputMode === 'area' ? 'area' : 'boards'
        ...
        if (hasDimensions) {
          ...
          const quote = await quoteConfigurationPricing({
            ...
            quantityBoards: inputMode === 'area' ? undefined : item.quantity,
            inputMode,
            targetAreaM2: inputMode === 'area' ? item.targetAreaM2 ?? item.quantity : undefined,
          })
          ...
          if (!quote) {
            ...
            const snapshotLineTotal = pricingSnapshot?.lineTotal
            const snapshotUnit =
              inputMode === 'area'
                ? pricingSnapshot?.pricePerM2Used
                : pricingSnapshot?.unitPrice
            const fallbackUnit =
              Number.isFinite(snapshotUnit) && (snapshotUnit as number) > 0
                ? (snapshotUnit as number)
                : Number.isFinite(item.basePrice) && Number(item.basePrice) > 0
            ...
            lineTotalCents =
              typeof snapshotLineTotal === 'number' && Number.isFinite(snapshotLineTotal) && snapshotLineTotal > 0
                ? centsFromEur(snapshotLineTotal)
                : centsFromEur(fallbackUnit * item.quantity)
            resolvedBy = { source: pricingSnapshot ? 'fallback_pricing_snapshot' : 'fallback_product_price' }
          } else {
            unitPriceEur = inputMode === 'area' ? quote.unitPricePerM2 : quote.unitPricePerBoard
            lineTotalCents = centsFromEur(quote.lineTotal)
        ...
        if (roleDiscount) {
          ...
          if (lineTotalCents !== null && item.quantity > 0) {
            lineTotalCents = centsFromEur(unitPriceEur * item.quantity)
          }
        }

        const unitPriceCents = centsFromEur(unitPriceEur)
        const resolvedLineTotalCents = lineTotalCents ?? unitPriceCents * item.quantity
