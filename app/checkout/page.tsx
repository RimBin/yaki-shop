...
export default function CheckoutPage() {
  ...
  const quantityLabel = currentLocale === 'lt' ? 'Kiekis' : 'Qty';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (quoteRes.ok) {
        ...
        if (quoteRes.status >= 500 || isMissingConfigPrice) {
      ...
      if (!response.ok) {
        const paymentError = typeof data?.error === 'string' ? data.error : undefined;
        const normalizedPaymentError = (paymentError || '').toLowerCase();
        const isRecoverablePayseraFailure =
          response.status >= 500 || normalizedPaymentError.includes('paysera konfigūracija nerasta');

        if (isRecoverablePayseraFailure) {
          console.warn('Paysera init unavailable; redirecting to order confirmation', {
            status: response.status,
            error: paymentError,
            orderId,
          });
          window.location.href = `/order-confirmation?provider=paysera&order_id=${encodeURIComponent(orderId)}`;
          return;
        }

        throw new Error(paymentError || t('errors.paymentSessionFailed'));
      }
  ...
  const getItemImage = (preferredImage: string | undefined, slug: string, id: string) => {
    if (typeof preferredImage === 'string' && preferredImage.trim()) return preferredImage;
  ...
  return (
    <main className="min-h-screen bg-[#E1E1E1]">
      ...
      <div className="w-full border-b border-[#BBBBBB]">
        <div className="max-w-[1440px] mx-auto px-[16px] sm:px-[40px] py-[32px]">
          <h1
                  ...
                  <div key={item.lineId} className="py-[16px]">
                    <div className="flex gap-[12px] sm:gap-[14px] w-full">
                      <div className="relative w-[84px] h-[96px] sm:w-[111px] sm:h-[125px] overflow-hidden shrink-0">
                        <SeoImage
                          src={getItemImage(item.image, item.slug, item.id)}
                          alt={buildProductImageSeo(locale === 'lt' ? 'lt' : 'en', { name: item.name }).alt}
                          title={buildProductImageSeo(locale === 'lt' ? 'lt' : 'en', { name: item.name }).title}
                          description={buildProductImageSeo(locale === 'lt' ? 'lt' : 'en', { name: item.name }).description}
                          fill
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-[8px] flex flex-col gap-[8px] sm:flex-row sm:items-end sm:justify-between">
                          <div className="flex flex-col gap-[4px]">
                            {(item.color || item.finish) && (
                              <p className="font-['Outfit'] font-normal leading-[1.3] text-[#BBBBBB] text-[12px]">
                                {(item.color ? `${t('summary.colorLabel')} ${item.color}` : '') +
                                  (item.color && item.finish ? ' • ' : '') +
                                  (item.finish ? `${t('summary.profileLabel')} ${item.finish}` : '')}
                              </p>
                            )}
                            ...
                            <p className="font-['Outfit'] font-normal leading-[1.3] text-[#BBBBBB] text-[12px]">
                              {quantityLabel}: {formatCartItemQuantityWithBoards(item, currentLocale)}
                            </p>
                          </div>
