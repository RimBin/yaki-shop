...
interface ProductPageProps {
  ...
  searchParams?: Promise<MaybeProductSearchParams>;
}
...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
  ...
  const canonical = absoluteUrl(productPath, currentLocale);
  ...
  const productTitle = currentLocale === 'lt'
    ? `${displayName} - Shou Sugi Ban produktas`
    : `${displayName} - Shou Sugi Ban product`;
  const seoDescription = truncateSeo(
    `${displayDescription ?? ''} ${
      currentLocale === 'lt'
        ? 'Peržiūrėkite kainą, spalvas ir profilius Yakiwood kataloge.'
        : 'Explore pricing, colors, and profiles in the Yakiwood catalog.'
    }`.trim()
  );
  const socialTitle = currentLocale === 'lt'
    ? `${displayName} | Degintos medienos produktas | Yakiwood`
    : `${displayName} | Charred wood product | Yakiwood`;
...
export default async function ProductPage({ params, searchParams }: ProductPageProps) {
  ...
  const productSchema = generateProductSchema({
    name: displayName,
    slug: slugForLocale,
    url: absoluteUrl(toLocalePath(`/products/${slugForLocale}`, currentLocale), currentLocale),
