...
import { buildProductImageSeo, buildUiImageSeo } from '@/lib/seo/images';

type SolutionType = 'facades' | 'terraces' | 'interior' | 'fences';

const localizedSolutionHashes: Record<'lt' | 'en', Record<SolutionType, string>> = {
  lt: {
    facades: 'fasadai',
    terraces: 'terasos',
    interior: 'interjeras',
...
function getSolutionHash(locale: 'lt' | 'en', type: SolutionType) {
  return localizedSolutionHashes[locale][type];
}

function getSolutionTypeFromHash(hash: string): SolutionType | null {
  const normalizedHash = hash.replace(/^#/, '').trim().toLowerCase();
  if (!normalizedHash) return null;

  for (const localeMap of Object.values(localizedSolutionHashes)) {
    for (const [solutionType, solutionHash] of Object.entries(localeMap) as Array<[SolutionType, string]>) {
      if (solutionHash === normalizedHash) return solutionType;
    }
  }
...
export default function SolutionsPage() {
  ...
  const scrollToApplicationCard = (behavior: ScrollBehavior) => {
    if (typeof window === 'undefined') {
      return;
    }

    const targetTop = applicationCardRef.current?.getBoundingClientRect().top;
    ...
    const peekOffset = window.innerWidth >= 1024 ? 20 : 12;

    window.scrollTo({
      top: Math.max(0, window.scrollY + targetTop - headerOffset - peekOffset),
      behavior,
    });
  };
  ...
  const ModernApplicationCard = ({
    ...
    const dataByType = {
        facades: {
          ...
          primaryCta: { href: '/produktai', label: t('cta.getOffer') },
          secondaryCta: { href: '/projektai', label: t('cta.seeProjects') },
          image: { src: getAsset('projectFacadeNordicCoastline'), alt: t('facades.images.wide') },
          badges: [
            { label: t('facades.badgesLabels.epd'), desc: t('facades.badges.epd') },
            { label: t('facades.badgesLabels.fsc'), desc: t('facades.badges.fsc') },
            { label: t('facades.badgesLabels.uv'), desc: t('facades.badges.uv') },
          ],
        },
        terraces: {
          bgColor: '#EFECE7',
          ...
          primaryCta: { href: '/produktai', label: t('cta.getOffer') },
          secondaryCta: { href: '/projektai', label: t('cta.seeProjects') },
          image: { src: assets.categories.terrace, alt: t('terraces.images.topLeft') },
          badges: [
            { label: t('terraces.badgesLabels.grip'), desc: t('terraces.badges.grip') },
            { label: t('terraces.badgesLabels.drain'), desc: t('terraces.badges.drain') },
            { label: t('terraces.badgesLabels.season'), desc: t('terraces.badges.season') },
          ],
        },
        interior: {
          bgColor: '#E9ECEF',
          ...
          primaryCta: { href: '/produktai', label: t('cta.getOffer') },
          secondaryCta: { href: '/projektai', label: t('cta.seeProjects') },
          image: { src: assets.categories.interior, alt: t('interior.images.topLeft') },
          badges: [
            { label: t('interior.badgesLabels.text'), desc: t('interior.badges.text') },
            { label: t('interior.badgesLabels.warm'), desc: t('interior.badges.warm') },
            { label: t('interior.badgesLabels.detail'), desc: t('interior.badges.detail') },
          ],
        },
        fences: {
          bgColor: '#EAEDE7',
          ...
          primaryCta: { href: '/produktai', label: t('cta.getOffer') },
          secondaryCta: { href: '/projektai', label: t('cta.seeProjects') },
          image: { src: assets.categories.fence, alt: t('fences.images.topLeft') },
          badges: [
            { label: t('fences.badgesLabels.long'), desc: t('fences.badges.long') },
            { label: t('fences.badgesLabels.clean'), desc: t('fences.badges.clean') },
            { label: t('fences.badgesLabels.care'), desc: t('fences.badges.care') },
          ],
        },
      };
      ...
      >
        ...
        >
          <SeoImage
            ...
            alt={buildUiImageSeo(currentLocale, { name: data.eyebrow, context: currentLocale === 'lt' ? 'Sprendimų sekcijos fonas' : 'Solutions section background' }).alt}
            title={buildUiImageSeo(currentLocale, { name: data.eyebrow, context: currentLocale === 'lt' ? 'Sprendimų sekcijos fonas' : 'Solutions section background' }).title}
            description={buildUiImageSeo(currentLocale, { name: data.eyebrow, context: currentLocale === 'lt' ? 'Sprendimų sekcijos fonas' : 'Solutions section background' }).description}
            fill
        ...
        <div className="relative p-[16px] md:p-[24px] lg:p-[40px]">
          <div className="grid gap-[24px] lg:grid-cols-[420px_1fr] lg:gap-[48px] items-start">
            ...
            <div className="grid gap-[12px] md:gap-[16px] lg:gap-[24px]">
              <div className="relative overflow-hidden rounded-[24px] h-[260px] md:h-[360px] lg:h-[540px]">
                <SeoImage
                  src={data.image.src}
                  alt={buildUiImageSeo(currentLocale, { name: data.image.alt, context: currentLocale === 'lt' ? 'Sprendimo nuotrauka' : 'Solution image' }).alt}
                  title={buildUiImageSeo(currentLocale, { name: data.image.alt, context: currentLocale === 'lt' ? 'Sprendimo nuotrauka' : 'Solution image' }).title}
                  description={buildUiImageSeo(currentLocale, { name: data.image.alt, context: data.description }).description}
                  fill
                  priority
                  className="object-cover"
              ...
              <div className="grid grid-cols-3 gap-[12px] md:gap-[16px] lg:gap-[24px]">
                {data.badges.map((b) => (
                  <div key={b.label} className="rounded-[24px] border border-[#BBBBBB] bg-white/60 p-[14px]">
  ...
  const scrollToSection = (id: SolutionType) => {
    ...
    // Keep URL shareable without changing the current scroll position.
    if (typeof window !== 'undefined') {
      window.history.replaceState(null, '', `#${getSolutionHash(currentLocale, id)}`);
    }
  };

  useEffect(() => {
    const hash = typeof window !== 'undefined' ? window.location.hash : '';
    const id = getSolutionTypeFromHash(hash);
    if (!id) return;

    // Wait until the layout is painted, then open the requested tab and jump to its card.
    const t = window.setTimeout(() => {
      setActiveFilter(id);
      setOpenAccordionIndex(0);
      window.requestAnimationFrame(() => {
        window.requestAnimationFrame(() => {
          scrollToApplicationCard('auto');
        });
      });
    }, 0);
    ...
    <div className="bg-transparent overflow-x-hidden">
      ...
      <InView className="hero-animate-root">
          <section className="max-w-[1440px] mx-auto px-[16px] md:px-[32px] lg:px-[40px] pt-[16px] lg:pt-[24px] pb-[48px] lg:pb-[72px]">
            ...
            <div ref={applicationCardRef} className="mb-[48px] lg:mb-[72px] hero-seq-item hero-seq-right" style={{ animationDelay: '320ms' }}>
              ...
              return (
                ...
                >
                  <div className="relative w-full h-[200px] lg:h-[240px] overflow-hidden rounded-[4px]">
                    <SeoImage
                      src={safeSrc}
                      alt={buildProductImageSeo(currentLocale, { name: product.name, shortDescription: product.usage }).alt}
                      title={buildProductImageSeo(currentLocale, { name: product.name, shortDescription: product.usage }).title}
                      description={buildProductImageSeo(currentLocale, { name: product.name, shortDescription: product.usage }).description}
                      fill
                      className="object-cover"
                    />
