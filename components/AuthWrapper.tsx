...
export default function AuthWrapper({ children }: { children: React.ReactNode }) {
  ...
  const shouldBypassGate = isDevelopment || !isPreviewGateEnabled;

  const [isAuthenticated, setIsAuthenticated] = useState(shouldBypassGate);
  const [isLoading, setIsLoading] = useState(!shouldBypassGate);
  ...
  useEffect(() => {
    ...
    setIsLoading(false);
  }, [shouldBypassGate, isPreviewGateEnabled, isDevelopment]);
  ...
  const isEvaluatorRoute = normalizedPathname === '/vertinimui';
  const noHeaderOffsetPrefixes = ['/', '/reset-password'];
