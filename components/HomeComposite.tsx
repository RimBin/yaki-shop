    ...
    <div className="bg-[#e1e1e1] relative w-[1440px] h-[11712px] mx-auto" data-name="1440px/Homepage" data-node-id="790:6589">
      ...
      <div className="absolute bg-[#161616] box-border flex gap-[200px] items-center justify-center left-1/2 px-[40px] py-[8px] top-0 translate-x-[-50%] w-[1440px]" data-name="Announcement bar">
        <div className="flex gap-[8px] items-center">
          <div className="relative size-[24px] overflow-clip">
            <img alt="Fast delivery icon" title="Fast delivery icon" src={img26} className="opacity-0" />
          </div>
      ...
      <div className="absolute border-b border-[#bbbbbb] flex items-center justify-between left-1/2 px-[40px] py-[16px] top-[24px] translate-x-[-50%] w-[1440px]" data-name="Header">
        <div className="h-[48px] w-[126px]">
          <img alt="Yakiwood logo" title="Yakiwood logo" src={img61} className="h-[48px] w-[126px]" />
        </div>
      ...
      <div className="absolute left-0 top-[24px] w-[1440px] h-[776px]" data-name="Hero">
        ...
        <img alt="Burnt wood hero background" title="Burnt wood hero background" src={imgMaskGroup} className="absolute inset-0 size-full" />
        <div className="absolute left-[40px] top-[259px] flex flex-col gap-[24px] w-[606px]">
          ...
          <div className="flex gap-[24px]">
            <img alt="Mastercard payment logo" title="Mastercard payment logo" src={imgMastercard2019Logo1} className="h-[21.856px] w-[35.366px]" />
            <img alt="Visa payment logo" title="Visa payment logo" src={imgVisa20211} className="h-[11.483px] w-[35.366px]" />
          </div>
        </div>
        {/* Certificates row */}
        <div className="absolute left-1/2 top-[800px] translate-x-[-50%] w-[1440px] flex gap-[150px] justify-center items-center px-[40px] py-[16px] bg-[#161616]">
          <img alt="Certification logo 1" title="Certification logo 1" src={img24} className="h-[53px] w-[46px]" />
          <img alt="Certification logo 2" title="Certification logo 2" src={img25} className="h-[24px] w-[81px]" />
          <img alt="Certification logo 3" title="Certification logo 3" src={img26} className="h-[47px] w-[35px]" />
          <img alt="Certification logo 4" title="Certification logo 4" src={img27} className="h-[41px] w-[80px]" />
          <img alt="Certification logo 5" title="Certification logo 5" src={img28} className="h-[48px] w-[53px]" />
          <img alt="Certification logo 6" title="Certification logo 6" src={img29} className="h-[27px] w-[38px]" />
        </div>
      </div>
          ...
          {[img25,img35,img36].map((imgSrc, idx) => (
            <div key={idx} className="bg-[#eaeaea] rounded-[8px] flex flex-col gap-[24px] pt-[24px] pb-[40px] px-[24px] w-[443px]">
              <div className="h-[311px] w-[395px] bg-[#eaeaea] overflow-hidden">
                <img alt={idx === 0 ? 'Spruce wood product finish' : 'Larch wood product finish'} title={idx === 0 ? 'Spruce wood product finish' : 'Larch wood product finish'} src={imgSrc} className="object-cover w-full h-full" />
              </div>
              <div className="flex flex-col gap-[16px] w-[383px]">
                ...
                <div className="flex gap-[8px] items-center flex-wrap">
                  {[img26,img27,img29,img30,img31,img32,img33,img34].map((c, colorIndex)=> <img key={c} alt={`Color option ${colorIndex + 1}`} title={`Color option ${colorIndex + 1}`} src={c} className="h-[32px] w-[32px] rounded" />)}
                </div>
              </div>
      ...
      <div className="absolute left-0 top-[3039px] w-[1440px]" data-name="Solution">
        ...
        <div className="absolute left-0 top-[225px] flex flex-col w-full">
          ...
          <div className="flex bg-[#161616] px-[40px] py-[24px] gap-[16px] items-start">
            ...
            <div className="h-[300px] w-[672px] rounded-[8px] overflow-hidden"><img alt="Burnt wood facade cladding" title="Burnt wood facade cladding" src={img58} className="h-full w-full object-cover" /></div>
            <div className="flex flex-col justify-between h-[300px] w-[288px] text-white">
              ...
              <button className="flex items-center gap-[8px] h-[24px] text-[12px] tracking-[0.6px] uppercase">Learn more <img alt="Arrow icon" title="Arrow icon" src={img59} className="h-[16px]" /></button>
            </div>
          </div>
          <div className="flex bg-[#e1e1e1] px-[40px] py-[24px] gap-[16px] items-start">
        ...
        <div className="absolute left-[40px] top-[144px] flex flex-wrap gap-[24px] w-[1360px]">
          {[img51,img52,img53,img54,img55,img56].map((p,i)=> (
            <div key={i} className="rounded-[8px] overflow-hidden bg-[#eaeaea]" style={{width: i<2?508: (i%3===2?328:328), height: i<2?520:330}}>
              <img alt={`Featured project ${i + 1}`} title={`Featured project ${i + 1}`} src={p} className="w-full h-full object-cover" />
            </div>
          ))}
        </div>
        <button className="absolute left-[calc(75%+164px)] top-[28px] h-[24px] text-[12px] tracking-[0.6px] uppercase flex items-center">View all projects <img alt="Arrow icon" title="Arrow icon" src={img11} className="h-[16px]" /></button>
      </div>
        ...
        <div className="absolute left-1/2 top-[144px] -translate-x-1/2 flex gap-[16px] w-[1440px] justify-center">
          {[img4,img5,img4].map((bg,i)=> (
            <div key={i} className={`rounded-${i===1?'[8px]':'[16px]'} p-[32px] h-[451px] w-[673px] flex flex-col justify-between ${i===1?'bg-[#161616]':'border border-[#bbbbbb] opacity-30'} relative`}>
              <img alt="Customer review background" title="Customer review background" src={bg} className="absolute inset-0 w-full h-full object-cover rounded-inherit" />
      ...
      <div className="absolute left-0 top-[7527px] w-[1440px]" data-name="About">
        ...
        <div className="absolute left-[40px] top-[224px] w-[1360px] flex">
          <img alt="Burnt wood craftsmanship detail" title="Burnt wood craftsmanship detail" src={img24} className="h-[175px] w-[175px] rounded-[8px]" />
          <img alt="About Yakiwood video preview" title="About Yakiwood video preview" src={img25} className="h-[623px] w-[672px] rounded-[8px] ml-[calc(25%+24px)]" />
          <div className="ml-[40px] w-[308px] flex flex-col gap-[40px]">
            ...
            <button className="h-[24px] text-[12px] tracking-[0.6px] uppercase flex items-center gap-[8px]">Read about us <img alt="Arrow icon" title="Arrow icon" src={img11} className="h-[16px]" /></button>
          </div>
        </div>
        ...
        <div className="absolute left-0 top-[947px] w-[1440px] flex gap-[16px] justify-center">
          {[img26,img27,img28,img29,img30,img31,img32].map((p, partnerIndex) => (
            <div key={p} className="bg-[#eaeaea] rounded-[8px] h-[210px] w-[213px] flex items-center justify-center">
              <img alt={`Partner logo ${partnerIndex + 1}`} title={`Partner logo ${partnerIndex + 1}`} src={p} className="max-h-[60px]" />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="absolute left-1/2 top-[9903px] translate-x-[-50%] w-[1440px]" data-name="CTA">        {/* Background image (shared with Solutions CTA) */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[625px] lg:w-[1099px] h-[599px] lg:h-[1053px] opacity-15 mix-blend-luminosity pointer-events-none">
          <Image src={assets.ctaBackground} alt="CTA background texture" title="CTA background texture" fill sizes="(min-width: 1024px) 1099px, 625px" className="object-contain" />
