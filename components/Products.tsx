...
import { buildProductImageSeo, buildUiImageSeo, type SeoLocale } from '@/lib/seo/images';
...
const thermoFinishes = assets.finishesPreview.thermo;
...
function MobileProductCard({
  ...
  return (
    <div className="bg-[#eaeaea] rounded-[8px] pt-[12px] pb-[16px] px-[16px] w-full h-full flex flex-col gap-[24px] items-center relative">
      {/* Background mask overlay */}
      <div className="absolute inset-0 pointer-events-none">
        <SeoImage
          src={imgMask}
          alt={buildUiImageSeo(currentLocale, { name: 'Yakiwood', context: currentLocale === 'lt' ? 'Produktų kortelės fonas' : 'Product card background' }).alt}
          title={buildUiImageSeo(currentLocale, { name: 'Yakiwood', context: currentLocale === 'lt' ? 'Produktų kortelės fonas' : 'Product card background' }).title}
          description={buildUiImageSeo(currentLocale, { name: 'Yakiwood', context: currentLocale === 'lt' ? 'Produktų kortelės fonas' : 'Product card background' }).description}
          fill
      ...
      <div className="relative w-[calc(100%+32px)] -mx-[16px] aspect-[4/3] shrink-0 z-10 md:mx-0 md:w-full md:px-0">
        <div className="relative w-full h-full rounded-[8px] overflow-hidden">
          <SeoImage
            ...
            alt={buildProductImageSeo(currentLocale, { name: product.title, shortDescription: product.description }).alt}
            title={buildProductImageSeo(currentLocale, { name: product.title, shortDescription: product.description }).title}
            description={buildProductImageSeo(currentLocale, { name: product.title, shortDescription: product.description }).description}
            fill
  ...
  <div className="w-full flex-1 flex flex-col gap-[16px] relative z-10">
        {/* Solution chips */}
        ...
        <div className="flex items-start justify-between gap-[12px] w-full">
          <p className="min-w-0 flex-1 font-['DM_Sans'] font-normal text-[24px] leading-[1.1] tracking-[-0.96px] text-[#161616] break-words">
            {product.title}
          </p>
          <p className="shrink-0 whitespace-nowrap text-right font-['DM_Sans'] font-normal text-[24px] leading-[1.1] tracking-[-0.96px] text-[#161616]">
            nuo {product.price} €
          </p>
        </div>
            ...
            {slides.map((slide, idx) => (
              ...
              >
                <span className="relative block w-full h-full rounded-full overflow-hidden">
                  <SeoImage
                    src={slide.image}
                    alt={buildUiImageSeo(currentLocale, { name: slide.label || product.title, context: currentLocale === 'lt' ? 'Spalvos variantas' : 'Color variant' }).alt}
                    title={buildUiImageSeo(currentLocale, { name: slide.label || product.title, context: currentLocale === 'lt' ? 'Spalvos variantas' : 'Color variant' }).title}
                    description={buildUiImageSeo(currentLocale, { name: slide.label || product.title, context: currentLocale === 'lt' ? 'Spalvos variantas' : 'Color variant' }).description}
                    fill
                    sizes="32px"
                    className="object-cover"
                  />
...
function DesktopProductCard({
  ...
  return (
    <div className="bg-[#eaeaea] rounded-[8px] pt-[24px] pb-[40px] px-[24px] flex-1 min-w-0 h-full flex flex-col gap-[24px] items-center relative">
      <div className="absolute inset-0 pointer-events-none">
        <SeoImage
          src={imgMask}
          alt={buildUiImageSeo(currentLocale, { name: 'Yakiwood', context: currentLocale === 'lt' ? 'Produktų kortelės fonas' : 'Product card background' }).alt}
      ...
      <div className="w-full flex-1 flex flex-col gap-[16px] relative z-10">
        ...
        <div className="flex items-start justify-between gap-[16px] w-full leading-[1.1] font-['DM_Sans'] font-normal text-[32px] tracking-[-1.28px] text-[#161616]">
          <p className="min-w-0 flex-1 break-words">{product.title}</p>
          <p className="shrink-0 whitespace-nowrap text-right">nuo {product.price} €</p>
        </div>
...
export default function Products() {
  ...
  const products = useMemo<ProductData[]>(
    () => [
      ...
      {
        id: 'thermo-wood',
        image: thermoFinishes.carbon,
        slides: [
          { image: thermoFinishes.carbon, label: 'Carbon', swatch: swatchIcons.carbon },
          { image: thermoFinishes.carbonLight, label: 'Carbon light', swatch: swatchIcons.carbonLight },
          { image: thermoFinishes.graphite, label: 'Graphite', swatch: swatchIcons.graphite },
          { image: thermoFinishes.darkBrown, label: 'Dark brown', swatch: swatchIcons.darkBrown },
          { image: thermoFinishes.silver, label: 'Silver', swatch: swatchIcons.silver },
          { image: thermoFinishes.black, label: 'Black', swatch: swatchIcons.black },
        ],
        title: t('items.thermo.title'),
        price: 89,
        description: t('items.thermo.description'),
        solutions: ['facade', 'terrace'],
      },
    ],
    [t]
  );
  ...
  return (
    <section className="w-full bg-[#E1E1E1]">
      ...
      <InView className="2xl:hidden hero-animate-root">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 items-stretch gap-[16px] px-[16px] sm:px-[24px] lg:px-[40px] pb-[24px]">
          {products.map((product, idx) => (
            <div
              key={idx}
              className="w-full h-full hero-seq-item hero-seq-right"
      ...
      <InView className="hidden 2xl:block max-w-[1440px] mx-auto px-[40px] relative hero-animate-root">
        ...
        <div className="mt-[58px] flex w-full items-stretch justify-between gap-[40px]">
          {products.map((product, idx) => (
            <div
              key={idx}
              className="flex min-h-full flex-1 min-w-0 hero-seq-item hero-seq-right"
              style={{ animationDelay: `${220 + idx * 180}ms` }}
            >
              <DesktopProductCard
                product={product}
