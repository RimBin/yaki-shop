...
import { canAccessAdminSection, type AdminAccessProfile } from '@/lib/admin/permissions';
...
function getActiveTabFromUrl(pathname: string, tabParam: string | null): AdminTabKey {
  ...
  if (p.startsWith('/admin/backups')) return 'backups';
...
export default function AdminPanelHeader({ access }: { access: AdminAccessProfile }) {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (postsRes.status === 'fulfilled' && Array.isArray(postsRes.value.posts)) {
          nextCounts.posts = postsRes.value.posts.length > 0 ? postsRes.value.posts.length : blogPosts.length;
        }
      } catch {
  ...
  const pageTitle = useMemo(() => {
    ...
    if (activeTab === 'backups') return t('breadcrumb.backups');
  ...
  const pageSubtitle = useMemo(() => {
    ...
    if (activeTab === 'backups') return t('main.subtitle');
  ...
  const breadcrumbsItems = useMemo(() => {
    ...
    else if (activeTab === 'backups') items.push({ label: t('breadcrumb.backups') });
  ...
  const tabs = useMemo<HeaderTab[]>(
    () => [
      ...
      { key: 'backups', label: t('tabs.backups'), href: toLocalePath('/admin/backups', currentLocale) },
  ...
  const visibleTabs = useMemo(
    () => tabs.filter((tab) => canAccessAdminSection(access, tab.key)),
    [access, tabs]
  );
