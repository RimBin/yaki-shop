    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          setErrorMessage('Sesija nebegalioja arba trūksta administratoriaus teisių. Prisijunkite iš naujo.');
          ...
          {errorMessage && (
            <div className="border-b border-[#E1E1E1] bg-[#FFF7ED] px-[16px] py-[12px]">
              ...
              <div className="mt-[4px] font-['Outfit'] text-[12px] text-[#9A3412]">
                {errorMessage.includes('administratoriaus teisių')
                  ? 'Produkcijoje šis puslapis veikia tik prisijungus su admin paskyra, kurios el. paštas yra `ADMIN_EMAILS` sąraše arba turi `role=admin`.'
