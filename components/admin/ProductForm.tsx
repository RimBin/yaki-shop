...
const ALLOWED_WOOD_TYPES = ['spruce', 'larch', 'thermo'] as const;
const ADMIN_WOOD_TYPES = ['spruce', 'larch', 'thermo'] as const;

const WOOD_NAME_LT: Record<(typeof ALLOWED_WOOD_TYPES)[number], string> = {
  spruce: 'Eglė',
  larch: 'Maumedis',
  thermo: 'Termo',
};

const WOOD_NAME_EN: Record<(typeof ALLOWED_WOOD_TYPES)[number], string> = {
  spruce: 'Spruce',
  larch: 'Larch',
  thermo: 'Thermo',
};
...
function buildLtProductName(usageType: string, woodType: string): string {
  ...
  const woodLt = WOOD_NAME_LT[(ALLOWED_WOOD_TYPES.includes(woodType as any) ? woodType : 'spruce') as (typeof ALLOWED_WOOD_TYPES)[number]];
...
function buildEnProductName(usageType: string, woodType: string): string {
  ...
  const woodEn = WOOD_NAME_EN[(ALLOWED_WOOD_TYPES.includes(woodType as any) ? woodType : 'spruce') as (typeof ALLOWED_WOOD_TYPES)[number]];
...
function woodLtFromType(woodType: string): string {
  return WOOD_NAME_LT[(ALLOWED_WOOD_TYPES.includes(woodType as any) ? woodType : 'spruce') as (typeof ALLOWED_WOOD_TYPES)[number]];
}

function woodEnFromType(woodType: string): string {
  return WOOD_NAME_EN[(ALLOWED_WOOD_TYPES.includes(woodType as any) ? woodType : 'spruce') as (typeof ALLOWED_WOOD_TYPES)[number]];
}
...
function formatAdminColorLabel(
  code: string,
  catalogColorOptions: Array<{ code: string; label: string }>,
  locale: 'lt' | 'en'
): string {
  const fromCatalog = catalogColorOptions.find((option) => option.code === code)?.label;
  ...
  if (!humanized) return code;
  return locale === 'lt' ? humanized : humanized;
}

function compareVariantDrafts(a: VariantDraft, b: VariantDraft): number {
  return (
...
const WOOD_TYPES = [
  ...
  { value: 'thermo', label: 'Termo' },
] as const satisfies ReadonlyArray<{ value: (typeof ADMIN_WOOD_TYPES)[number]; label: string }>;
...
export default function ProductForm({ product, mode }: Props) {
  ...
  const isLegacyThermoProduct = woodType === 'thermo';
  ...
  useEffect(() => {
    ...
    const loadVariantProducts = async () => {
      ...
      const { data, error } = await supabase
        .from('products')
        .select('id,name,slug,sku,stock_quantity,base_price,sale_price,is_active,width,depth,height')
        .ilike('slug', `${baseSlug}--%`)
        .neq('id', product.id)
  ...
  const generateVariantDrafts = () => {
    ...
    setVariantDrafts(drafts.sort(compareVariantDrafts));
  };
              ...
              <div className="mt-3 grid grid-cols-1 gap-3">
                <div>
                  ...
                  >
                    ...
                    <option value="thermo">{t('options.woodTypes.thermo')} (thermo)</option>
                  </select>
                </div>
              ...
              <div>
                ...
                {lockBaseFields ? (
                  <p className="text-xs text-[#7C7C7C] mt-1">
                    Vardas generuojamas automatiškai pagal pritaikymą ir medienos tipą.
                  </p>
                ) : null}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  {isLegacyThermoProduct ? (
                    <option value="thermo">Termo (legacy)</option>
                  ) : null}
                </select>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {variant.image_url && (
                      <img
                        ...
                        alt={variant.name ? `${variant.name} preview` : 'Variant preview'}
                        title={variant.name ? `${variant.name} preview` : 'Variant preview'}
                          ...
                          {libraryIsLoading ? (
                            ...
                            <div className="flex flex-wrap gap-2">
                              {libraryAssets.map((asset, assetIndex) => (
                                ...
                                >
                                  <img src={asset.url} alt={`${label} library image ${assetIndex + 1}`} title={`${label} library image ${assetIndex + 1}`} className="h-16 w-16 object-cover" />
                                </button>
                              ))}
                            </div>
                          )}
                  ...
                  {variantDrafts.map((draft) => (
                    <tr key={draft.id} className="border-t border-[#E1E1E1]">
                      <td className="px-3 py-2">
                        {formatAdminColorLabel(draft.color, catalogColorOptions, locale as 'lt' | 'en')}
                      </td>
                      <td className="px-3 py-2">
                        {resolveProfileLabel(
                          draft.profile,
                  ...
                  <table className="w-full text-sm border border-[#E1E1E1]">
                    <thead className="bg-[#E1E1E1]">
                      <tr>
                        <th className="text-left px-3 py-2">Spalva</th>
                        <th className="text-left px-3 py-2">Profilis</th>
                        <th className="text-left px-3 py-2">Dydis</th>
                      ...
                      {variantProducts.map((item: any) => (
                        <Fragment key={item.id}>
                          <tr className="border-t border-[#E1E1E1]">
                            <td className="px-3 py-2">
                              {(() => {
                                ...
                                return parsed
                                  ? formatAdminColorLabel(parsed.color, catalogColorOptions, locale as 'lt' | 'en')
                                  : '-';
                              })()}
                            </td>
                            <td className="px-3 py-2">
                              {(() => {
                                const parsed = parseStockItemSlug(String(item.slug || ''));
                                if (!parsed) return '-';
                                return resolveProfileLabel(parsed.profile, undefined, undefined, locale as 'lt' | 'en');
                              })()}
                            </td>
                            <td className="px-3 py-2">
                              {(() => {
                                const parsed = parseStockItemSlug(String(item.slug || ''));
                                return parsed?.size ? parsed.size.replace(/x/gi, '×') : '-';
                              })()}
                            </td>
                          ...
                          {expandedVariantId === item.id && (
                            <tr className="border-t border-[#E1E1E1]">
                              <td colSpan={10} className="px-3 py-4 bg-white">
          ...
          {variantType === 'color' && (
            <div>
              ...
              {imageUrl ? (
                <img
                  src={imageUrl}
                  alt={name ? `${name} preview` : 'Variant image preview'}
                  title={name ? `${name} preview` : 'Variant image preview'}
