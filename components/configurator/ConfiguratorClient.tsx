...
export default function ConfiguratorClient({ productSlug, presetSlug }: ConfiguratorClientProps) {
  ...
  const imageSeo = buildUiImageSeo(currentLocale, {
    name: 'Shou Sugi Ban',
    context: currentLocale === 'lt' ? '2D konfiguratoriaus lentų peržiūra' : '2D configurator board preview',
    description: currentLocale === 'lt' ? 'Shou Sugi Ban dailylenčių 2D peržiūra konfiguratoriuje' : 'Shou Sugi Ban cladding 2D preview in the configurator',
  });
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_520px] gap-[16px] lg:gap-[24px]">
      ...
      <div className="relative w-full h-[420px] lg:h-[729px] rounded-[8px] overflow-hidden bg-[#EAEAEA]">
        ...
        {config.view === '2d' ? (
          <Image2D alt={imageSeo.alt} title={imageSeo.title} description={imageSeo.description} />
        ) : (
