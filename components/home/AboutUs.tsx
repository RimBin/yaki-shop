        ...
        {repeated.map((name, index) => (
          ...
          >
            {logo ? (
              <SeoImage
                src={logo.src}
                alt={buildUiImageSeo(locale, {
                  name: logo.alt,
                  context: locale === 'lt' ? 'Partnerio logotipas' : 'Partner logo',
                }).alt}
                title={buildUiImageSeo(locale, {
                  name: logo.alt,
                  context: locale === 'lt' ? 'Partnerio logotipas' : 'Partner logo',
                }).title}
                description={buildUiImageSeo(locale, {
                  name: logo.alt,
                  context: locale === 'lt' ? 'Partnerio logotipas' : 'Partner logo',
                }).description}
                width={160}
    ...
    <section id="about-us" className="w-full bg-[#E1E1E1]">
      ...
      <InView className="xl:hidden pt-[64px] md:pt-[80px] lg:pt-[120px] pb-0 hero-animate-root">
        ...
        <div className="px-[16px] md:px-[32px] flex flex-col gap-[24px]">
          <div className="flex items-start gap-[16px] hero-seq-item hero-seq-right" style={{ animationDelay: '220ms' }}>
            <div className="relative w-[96px] h-[96px] rounded-[8px] overflow-hidden shrink-0">
              <SeoImage
                ...
                alt={buildUiImageSeo(currentLocale, {
                  name: 'Yakiwood',
                  context: currentLocale === 'lt' ? 'Apie mus nuotrauka' : 'About us image',
                }).alt}
            ...
            <div className="relative w-full h-[220px] md:h-[360px] rounded-[8px] overflow-hidden">
              <SeoImage
                src={imgAboutVideoThumb}
                alt={buildUiImageSeo(currentLocale, {
                  name: currentLocale === 'lt' ? 'Yakiwood vaizdo įrašo peržiūra' : 'Yakiwood video preview',
                  context: currentLocale === 'lt' ? 'Apie mus video' : 'About us video',
                }).alt}
                title={buildUiImageSeo(currentLocale, {
                  name: currentLocale === 'lt' ? 'Yakiwood vaizdo įrašo peržiūra' : 'Yakiwood video preview',
                  context: currentLocale === 'lt' ? 'Apie mus video' : 'About us video',
        ...
        <div className="mt-[40px] hero-seq-item hero-seq-right" style={{ animationDelay: '760ms' }}>
          ...
          <PartnersMarquee items={partnerNames} locale={currentLocale} durationSeconds={22} fullBleed />
        </div>
      </InView>
      ...
      <InView className="hidden xl:block max-w-[1440px] mx-auto px-[40px] pt-[200px] pb-[120px] hero-animate-root">
        ...
        <div className="mt-[24px] hero-seq-item hero-seq-right" style={{ animationDelay: '980ms' }}>
          <PartnersMarquee items={partnerNames} locale={currentLocale} durationSeconds={30} fullBleed />
        </div>
      </InView>
