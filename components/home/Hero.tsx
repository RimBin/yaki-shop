...
export default async function Hero() {
  ...
  const certificationLabels = ['FSC', 'Accoya', 'PEFC', 'EPD', 'ISO', 'CE'];
  const heroBackgroundSeo = buildUiImageSeo(currentLocale, {
    name: 'Yakiwood',
    context: currentLocale === 'lt' ? 'Hero fonas' : 'Hero background',
    description: currentLocale === 'lt' ? 'Pagrindinio puslapio hero fonas' : 'Homepage hero background',
  });
  const heroProductSeo = buildUiImageSeo(currentLocale, {
    name: tHero('productName'),
    context: currentLocale === 'lt' ? 'Hero produkto nuotrauka' : 'Hero product image',
  });
    ...
    <section className="w-full bg-[#E1E1E1] md:bg-[#EAEAEA] relative overflow-hidden xl:pb-0 xl:min-h-screen">
      ...
      <div className="absolute inset-0 hidden md:block z-0 pointer-events-none">
        <SeoImage
          ...
          alt={heroBackgroundSeo.alt}
          title={heroBackgroundSeo.title}
          description={heroBackgroundSeo.description}
          fill
            ...
            <div className="absolute inset-x-0 top-0 h-[390px] flex items-center justify-center md:inset-0 md:h-full">
              <div className="rotate-[333.068deg] w-full px-4 flex items-center justify-center">
                <div className="w-full max-w-[510px] aspect-[317/185] relative">
                  <SeoImage
                    src={imgProductImage}
                    alt={heroProductSeo.alt}
                    title={heroProductSeo.title}
                    description={heroProductSeo.description}
                    fill
                  ...
                  >
                    <SeoImage
                      ...
                      alt={buildUiImageSeo(currentLocale, {
                        name: currentLocale === 'lt' ? `Spalvų paletė ${i + 1}` : `Color palette ${i + 1}`,
                        context: currentLocale === 'lt' ? 'Hero spalvų pavyzdys' : 'Hero color swatch',
                      }).alt}
            ...
            {certifications.map((src, index) => (
              <div key={src} className="relative h-[24px] flex-1 min-w-0">
                <SeoImage
                  src={src}
                  alt={buildUiImageSeo(currentLocale, {
                    name: certificationLabels[index] ?? `Certification ${index + 1}`,
                    context: currentLocale === 'lt' ? 'Sertifikato logotipas' : 'Certification logo',
                  }).alt}
                  title={buildUiImageSeo(currentLocale, {
                    name: certificationLabels[index] ?? `Certification ${index + 1}`,
                    context: currentLocale === 'lt' ? 'Sertifikato logotipas' : 'Certification logo',
                  ...
                  fill
                  className="object-contain"
                  sizes="(max-width: 1024px) 16vw, 96px"
                />
              </div>
            ))}
          </div>
      ...
      <InView className="hidden xl:block relative z-10 w-full min-h-[861px] 2xl:min-h-[calc(100vh-96px)] hero-animate-root is-inview">
        <div data-hero-main className="relative max-w-[1440px] mx-auto h-[861px] 2xl:h-[calc(100vh-96px)]">
          <div className="absolute left-[40px] top-[220px] 2xl:top-[clamp(220px,20vh,300px)] flex max-w-[606px] flex-col gap-[24px] z-10 hero-seq-item hero-seq-right hero-ease-in hero-seq-1">
            <p
            ...
            <div className="flex flex-col gap-[24px] overflow-clip">
              ...
              <PaymentLogos variant="default" className="w-[280px]" />
            </div>
          </div>

          <div className="absolute right-[40px] top-[108px] 2xl:top-[clamp(108px,14vh,180px)] z-10 hero-seq-item hero-seq-right hero-seq-2">
