...
export default function ImageGallery({ images, productName }: ImageGalleryProps) {
  ...
  const selectedImageSeo = buildProductImageSeo(currentLocale, {
    name: selectedImage?.alt || productName,
    attributeLabel: sortedImages.length > 1 ? `${currentLocale === 'lt' ? 'Nuotrauka' : 'Image'} ${selectedIndex + 1}` : undefined,
  });
    ...
    <div className="w-full space-y-4">
      ...
      >
        <SeoImage
          ...
          description={selectedImageSeo.description}
          fill
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-[#BBBBBB] scrollbar-track-transparent">
          {sortedImages.map((image, index) => (
            ...
            >
              <SeoImage
                src={image.url}
                alt={buildProductImageSeo(currentLocale, {
                  name: image.alt || productName,
                  attributeLabel: `${currentLocale === 'lt' ? 'Nuotrauka' : 'Image'} ${index + 1}`,
                }).alt}
                title={buildProductImageSeo(currentLocale, {
                  name: image.alt || productName,
                  attributeLabel: `${currentLocale === 'lt' ? 'Nuotrauka' : 'Image'} ${index + 1}`,
                }).title}
                description={buildProductImageSeo(currentLocale, {
                  name: image.alt || productName,
                  attributeLabel: `${currentLocale === 'lt' ? 'Nuotrauka' : 'Image'} ${index + 1}`,
                }).description}
                fill
