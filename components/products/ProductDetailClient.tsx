...
function getEnglishColorLabel(color: Pick<ProductColorVariant, 'name' | 'nameEn'> | null | undefined): string {
  if (!color) return '';
  if (typeof color.nameEn === 'string' && color.nameEn.trim()) return color.nameEn;
  if (typeof color.name === 'string' && color.name.trim()) return localizeColorLabel(color.name, 'en');
  return '';
}

function getColorMatchTokens(color: ProductColorVariant): string[] {
  const tokens = [color.name, color.nameLt, color.nameEn]
    .filter((value): value is string => Boolean(value))
    .map((value) => normalizeColorKey(value))
    .filter(Boolean);

  if (typeof color.id === 'string' && color.id.startsWith('stock-color:')) {
    const stockToken = normalizeColorKey(color.id.replace('stock-color:', ''));
    if (stockToken) tokens.push(stockToken);
  }

  return Array.from(new Set(tokens));
}

function findColorOptionByToken(colorOptions: ProductColorVariant[], rawToken?: string | null): ProductColorVariant | null {
  const token = normalizeColorKey(rawToken || '');
  if (!token) return null;

  return (
    colorOptions.find((color) => getColorMatchTokens(color).some((candidate) => candidate === token)) ??
    colorOptions.find((color) => getColorMatchTokens(color).some((candidate) => candidate.includes(token) || token.includes(candidate))) ??
    null
  );
}
  ...
  for (const item of items) {
    ...
    if (colorToken && !colors.has(colorToken)) {
      colors.set(colorToken, {
        ...
        name: localizeColorLabel(parsed.color, 'en'),
        hex: undefined,
...
export default function ProductDetailClient({ product, relatedProducts = [], prefetchedStockItems = [] }: ProductDetailClientProps) {
  ...
  const colorOptions = useMemo<ProductColorVariant[]>(() => {
    const base = (product.colors || []).map((color, index) => ({
      ...
      name: getEnglishColorLabel(color) || color.name,
  ...
  }, [product.colors, product.id, stockDerivedOptions?.colors]);
  ...
  const urlColorSelection = useMemo(() => {
    ...
    if (urlColorToken) return findColorOptionByToken(colorOptions, urlColorToken);
    return null;
  ...
  const attributeSummary = useMemo(() => {
    ...
    const colorName =
      ...
      (parsed?.color ? localizeColorLabel(parsed.color, 'en') : '');
  ...
  useEffect(() => {
    ...
    if (!initialColor && urlColorToken && colorOptions.length) {
      initialColor = findColorOptionByToken(colorOptions, urlColorToken);
    }
    if (!initialColor && presetColorKey && colorOptions.length) {
      initialColor = findColorOptionByToken(colorOptions, presetColorKey);
    }
  ...
  useEffect(() => {
    ...
    if (urlColorToken) {
      const match = findColorOptionByToken(colorOptions, urlColorToken);
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      for (const color of colorOptions) {
        preloadColorImage(
          typeof color.productImage === 'string'
            ? color.productImage
  ...
  const colorPreviewImage =
    typeof effectiveSelectedColor?.productImage === 'string'
      ? effectiveSelectedColor.productImage
      : typeof effectiveSelectedColor?.image === 'string'
        ? effectiveSelectedColor.image
        : null;
  ...
  const productImageSeo = buildProductImageSeo(currentLocale, {
    name: localizedDisplayName,
    attributeLabel: attributeSummary,
    shortDescription: displayDescription || undefined,
  });
        ...
        <div className="flex flex-col lg:flex-row gap-[24px] lg:gap-[36px]">
              ...
              {show3D ? (
                <div className="absolute inset-0">
                  <DynamicKonfiguratorius3D
                    ...
                    isLoading={loading3D}
                    mode="viewport"
                    modelUrl={getProductModelUrl({
                      slug: product.slug,
                      category: product.category,
                      woodType: product.woodType,
                      profile: selectedFinish?.code ?? selectedFinish?.name,
                    })}
                    visualDimensionsMm={{
                      widthMm: selectedWidthMm,
                      lengthMm: selectedLengthMm,
                    ...
                    className="h-full"
                    canvasClassName="h-full !border-0 !rounded-none"
                  />
                </div>
              ) : (
                <SeoImage
                  src={optimizedActiveImage}
                  alt={productImageSeo.alt}
                  title={productImageSeo.title}
                  description={productImageSeo.description}
                  fill
          ...
          <div className="flex-1 flex flex-col gap-[24px] order-2 lg:order-2 hero-seq-item hero-seq-right" style={{ animationDelay: '200ms' }}>
            ...
            <div className="lg:hidden">
              <div
                className={`relative w-full bg-[#EAEAEA] rounded-[4px] overflow-hidden ${
                  show3D
                    ? 'h-[58svh] min-h-[320px] max-h-[440px] sm:h-[62svh] sm:min-h-[360px] sm:max-h-[520px]'
                    : 'aspect-square'
                }`}
              >
                {unitPricePerBoardDisplay ? (
                        ...
                        {swatchSrc ? (
                          <SeoImage
                            ...
                            alt={buildUiImageSeo(currentLocale, {
                              name: color.name,
                              context: currentLocale === 'lt' ? 'Spalvos mėginys' : 'Color swatch',
                            }).alt}
                ...
                <div className="flex gap-[8px] flex-wrap">
                  {widthOptionsMm.map((width) => {
                    ...
                    <button
                      ...
                      disabled={!isAvailable}
                      className={`relative h-[48px] min-w-[calc(50%-4px)] flex-1 px-[16px] sm:min-w-0 sm:flex-none flex items-center justify-center font-['Outfit'] font-normal text-[14px] tracking-[0.42px] uppercase transition-colors ${
                        isActive
                    ...
                    {profileOptions.map((finish, index) => {
                      ...
                      return (
                        <button
                          ...
                          disabled={!isAvailable}
                          className={`relative h-[48px] min-w-[120px] flex-1 sm:w-[83px] sm:min-w-0 sm:flex-none flex items-center justify-center transition-colors ${
                            active
                              ? 'bg-[#161616]'
                              : isAvailable
        ...
        <section className="max-w-[1440px] mx-auto px-[16px] sm:px-[24px] lg:px-[40px] py-[40px] lg:py-[80px]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-[16px] md:gap-x-[19px] gap-y-[40px] md:gap-y-[56px]">
            {relatedProducts.slice(0, 4).map((item) => {
              ...
              const relatedImageSeo = buildProductImageSeo(currentLocale, {
                name: localizedDisplayName,
                attributeLabel,
              });
                ...
                >
                  <div className="relative w-full h-[250px] border border-[#161616] border-opacity-30 overflow-hidden">
                    <SeoImage
                      ...
                      alt={relatedImageSeo.alt}
                      title={relatedImageSeo.title}
                      description={relatedImageSeo.description}
                      fill
