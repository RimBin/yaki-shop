...
import { formatCartItemQuantityWithBoards } from '@/lib/cart/display';
...
export default function CartSidebar({ isOpen, onClose }: CartSidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className="fixed top-0 right-0 h-full w-full sm:w-[552px] bg-[#E1E1E1] z-50 transform transition-transform duration-300 translate-x-0"
        role="dialog"
        aria-modal="true"
                ...
                <div className="space-y-[16px]">
                  {items.map((item) => {
                    ...
                    const imageSeo = buildProductImageSeo(currentLocale, {
                      name: item.name,
                      attributeLabel: item.color,
                      shortDescription: item.finish,
                    });

                    return (
                    <div key={item.addedAt ?? item.lineId} className="flex gap-[16px] pb-[16px] border-b border-[#BBBBBB]" data-testid="cart-item">
                      {/* Image */}
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        <div>
                          ...
                          {(item.color || item.finish) && (
                            <div className="space-y-[4px]">
                              ...
                              {item.finish && (
                                <p className="font-['Outfit'] font-light text-[14px] leading-[1.3] text-[#161616]">
                                  {t('cart.profile')} {formatFinishLabel(item.finish)}
                                </p>
                              )}
                            </div>
                          )}
                          ...
                          <div className="mt-[10px]">
                            ...
                            <p className="mt-[2px] font-['Outfit'] font-normal text-[12px] leading-[1.4] text-[#535353]">
                              {t('cart.quantity') ?? 'Kiekis'}: {formatCartItemQuantityWithBoards(item, currentLocale)}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-[12px] sm:flex-row sm:items-center sm:justify-between">
                          ...
                          {/* Quantity Controls */}
                          <div className="flex flex-wrap items-center gap-x-[8px] gap-y-[10px] sm:justify-end">
                            <button
                              onClick={() => {
                                const isArea = item.inputMode === 'area';
                                const step = isArea ? 0.01 : 1;
                                const min = isArea ? 0.01 : 1;
                                const next = isArea
                                  ? Math.max(min, Math.round((item.quantity - step) * 100) / 100)
                                  : Math.max(min, item.quantity - step);
                                updateQuantity(item.lineId, next);
                              }}
                              aria-label={`${currentLocale === 'lt' ? 'Sumažinti kiekį' : 'Decrease quantity'} ${item.name}`}
                              className="w-[24px] h-[24px] flex items-center justify-center hover:opacity-70 transition-opacity"
                            >
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="flex min-w-0 items-center justify-center gap-[6px] whitespace-nowrap">
                                  <input
                                    inputMode="decimal"
                                    type="number"
                                    min={min}
                                    ...
                                    onFocus={() => {
                                      setDraftQuantities((prev) => ({
                                        ...prev,
                                        [item.lineId]: isArea ? Number(item.quantity).toFixed(2) : String(item.quantity),
                                      }));
                                    }}
                                    onChange={(e) => {
                                    ...
                                    className="h-[28px] w-[58px] rounded-[6px] border border-[#BBBBBB] bg-[#EAEAEA] px-[8px] font-['Outfit'] font-normal text-[14px] leading-[1.5] text-[#161616] text-center"
                                    aria-label={`${t('cart.quantity') ?? 'Quantity'} ${item.name}`}
                                  />
                            ...
                            <button
                              ...
                              aria-label={`${currentLocale === 'lt' ? 'Padidinti kiekį' : 'Increase quantity'} ${item.name}`}
                              className="w-[24px] h-[24px] flex items-center justify-center hover:opacity-70 transition-opacity"
                            >
                              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button
                              ...
                              className="font-['Outfit'] font-normal text-[12px] leading-[1.2] tracking-[0.6px] uppercase text-[#535353] hover:text-[#161616] transition-colors sm:ml-[8px]"
                            >
                              {t('cart.remove')}
                            </button>
                          </div>
