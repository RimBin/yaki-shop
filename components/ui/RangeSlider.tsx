...
export default function RangeSlider({
  ...
  }, [dragging, getValueFromPosition, localValue, max, min, onChange, step]);
  ...
  return (
    <div className={`w-full ${className}`}>
      <div className="relative h-2 mb-7" ref={trackRef}>
        {/* Track background */}
      ...
      {/* Value labels */}
      <div className="flex items-center justify-between gap-4 text-[13px] font-['DM_Sans'] text-[#161616]">
        <span className="shrink-0 whitespace-nowrap">{formatLabel(localValue[0])}</span>
        <span className="shrink-0 whitespace-nowrap">{formatLabel(localValue[1])}</span>
      </div>
    </div>
  );
}
