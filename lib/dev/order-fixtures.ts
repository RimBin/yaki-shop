...
export type DevelopmentOrderRecord = {
  id: string;
  order_number: string;
...
export type DevelopmentInvoiceRecord = {
  id: string;
...
export function getDevelopmentOrdersAndInvoices(now = new Date()): {
  orders: DevelopmentOrderRecord[];
  invoices: DevelopmentInvoiceRecord[];
} {
  ...
  const orders: DevelopmentOrderRecord[] = [
    {
      id: orderId,
  ...
  const invoices: DevelopmentInvoiceRecord[] = [
    {
      id: 'demo-invoice-001',
...
export function getDevelopmentInvoiceById(id: string, now = new Date()): DBInvoice | null {
  const { invoices } = getDevelopmentOrdersAndInvoices(now);
  const match = invoices.find((invoice) => invoice.id === id);
