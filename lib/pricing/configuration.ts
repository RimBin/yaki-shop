...
async function resolveFallbackUnitPricePerM2(selectors: {
  ...
  const seedProduct = seedProducts.find((product) => String(product.id) === productId)

  if (!client) {
    if (!seedProduct) return null
    return {
      unitPricePerM2: toNumber(seedProduct.basePrice),
      matchedRowId: `seed-products:${seedProduct.id}`,
      specificity: 0,
    }
  }
