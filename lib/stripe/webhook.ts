...
type StripeConstructor = new (
  apiKey: string,
  config: { apiVersion: '2025-11-17.clover' }
) => Stripe;
...
function getStripeClient(): Stripe | null {
  ...
  const StripeCtor = require('stripe') as StripeConstructor;
...
export async function handleStripeWebhook(req: NextRequest): Promise<NextResponse> {
  ...
  if (!stripe || !webhookSecret) {
    reportMissingPaymentEnv('stripe.webhook', ['STRIPE_SECRET_KEY', 'STRIPE_WEBHOOK_SECRET']);
  ...
  if (!signature) {
    logPaymentProof('warn', 'stripe.webhook.missing_signature');
  ...
  try {
    ...
    logPaymentProof('warn', 'stripe.webhook.invalid_signature', {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      ...
      if (paymentStatus !== 'paid') {
        logPaymentProof('warn', 'stripe.webhook.session_not_paid', {
          eventId: event.id,
      ...
      if (!customerEmail) {
        logPaymentProof('warn', 'stripe.webhook.missing_customer_email', {
          eventId: event.id,
          sessionId: session.id,
          orderId,
        });
      ...
      let items: StripeWebhookItem[] = [];
      try {
        const parsed = JSON.parse(itemsJson);
        items = Array.isArray(parsed) ? (parsed as StripeWebhookItem[]) : [];
      } catch {
        logPaymentProof('warn', 'stripe.webhook.invalid_items_metadata', {
          eventId: event.id,
          sessionId: session.id,
          orderId,
        });
      ...
      if (orderId && typeof orderId === 'string' && orderId.trim().length > 0) {
        ...
        if (existing) {
          ...
          if (paidCents !== null && Number.isFinite(expectedCents) && Math.abs(paidCents - expectedCents) > 1) {
            logPaymentProof('error', 'stripe.webhook.amount_mismatch', {
              eventId: event.id,
              sessionId: session.id,
              orderId: existing.id,
              paidCents,
          ...
          if (Array.isArray(existing.items) && existing.items.length > 0) {
            items = existing.items as StripeWebhookItem[];
          }
        }
      }

      // Prices in cart/checkout are treated as GROSS (VAT included).
      const grossTotal = items.reduce((sum: number, item) => sum + item.quantity * item.basePrice, 0);
      ...
      if (!order) {
        ...
        if (!order) {
          logPaymentProof('error', 'stripe.webhook.order_create_failed', {
            eventId: event.id,
            sessionId: session.id,
            customerEmail,
          });
      ...
      if (!emailResult.success) {
        logPaymentProof('warn', 'stripe.webhook.email_failed', {
          eventId: event.id,
          orderId: order.id,
        ...
        logPaymentProof('info', 'stripe.webhook.fulfilled', {
          eventId: event.id,
          sessionId: session.id,
          orderId: order.id,
          orderNumber: orderNumber || order.order_number,
      ...
      logPaymentProof('error', 'stripe.webhook.exception', {
        eventId: event.id,
